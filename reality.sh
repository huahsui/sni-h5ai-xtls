...
sleep 1
wget -N --no-check-certificate -q -O /html/we.dog/$UUID.yaml "https://raw.githubusercontent.com/huahsui/tcp-xtls/gh-pages/clash.yaml" && sed -i '48 i\  - {name: tcp+xtls, server: $DOMIN, port: 443, type: vless, uuid: $UUID, network: tcp, tls: true, udp: true, flow: xtls-rprx-vision, servername: $DOMIN, reality-opts: {public-key: $Two, short-id: 12345678}, client-fingerprint: chrome}' /html/we.dog/$UUID.yaml
sleep 1
clear
...
echo "----------------------------------------------------------------------------------------------------------------------------------------------"
echo
echo "  客户端配置文件在 https://$DOMIN/client.json 请直接下载"
echo
echo "  分享链接：vless://$UUID@$Zero:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=$DOMIN&fp=chrome&pbk=$Two&sid=12345678&spx=%2F&type=tcp&headerType=none#reality"
echo
